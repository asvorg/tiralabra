...
impl Ui {
    ...
    pub fn demo() -> (){
        ...
        println!();
        let message_uint_decrypted: num_bigint::BigUint = Decrypt::decrypt(message_uint_encrypted, n_copy, d);
        ...
        println!();
        let message_uint_decrypted_2: num_bigint::BigUint = Decrypt::decrypt(message_uint_encrypted_2, n_copy3, d_copy);
    ...
    fn encrypt_and_print() -> () {
        ...
        io::stdin().read_line(&mut message).expect("Failed to read line");
        let message_uint: num_bigint::BigUint = Encrypt::convert_text_to_int(&message);
        let message_uint_encrypted: num_bigint::BigUint = Encrypt::encrypt(message_uint.clone(),n.clone(),e.clone());
        println!("Encrypted message: {}",message_uint_encrypted);
            }
    }
    #[cfg(not(tarpaulin_include))]
    ...
    fn demo_with_user_keys() -> () {
        ...
        let message_uint_encrypted: num_bigint::BigUint = Encrypt::encrypt(message_uint.clone(),n.clone(),e.clone());
        println!("Encrypted message: {}",message_uint_encrypted);
        let message_uint_decrypted: num_bigint::BigUint = Decrypt::decrypt(message_uint_encrypted, n_2, d);
        let message_decrypted: String = Decrypt::convert_int_to_text(&message_uint_decrypted);
        println!("Message decrypted and converted: {}",message_decrypted);
    }
